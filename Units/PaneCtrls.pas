...
uses
  Classes, Controls, Forms, Windows, Messages, Graphics, ImgList,
...
{ TCustomPaneControl }
  ...
  TCustomPaneControl = class(TFramedWindow)
  private
...
begin
  ...
  for I := Low(TPaneButton) to High(TPaneButton) do
    if FShowButtons and PtInRect(ButtonRect[I], Point(X, Y)) then
    begin
      Result := I;
      Break;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      OverwriteObject(DC, GetPen(clThemeBorder));
    ...
    begin
      ...
      begin
        ...
        if PaneIndex > FLeftIndex then
          Inc(Right, CalcCaptionSize(DC, Pane.Caption).cX + 16);
      end;
      if I = Index then
      begin
        if Pane.Opened then
        begin
          Result.Left := Right - CalcCaptionSize(DC, Pane.Caption).cX - 16;
          OffsetRect(Result, 8, 0);
          Bottom := FHeaderHeight + 4;
          Top := CalcCaptionSize(DC, 'Wg').cY;
...
procedure TCustomPaneControl.SetShowButtons(const Value: Boolean);
begin
