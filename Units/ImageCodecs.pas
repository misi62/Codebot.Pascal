...
const
    ...
    WICMetadataCreationMask = LongInt($FFFF0000);
...
type
  ...
  ['{22F55882-280B-11D0-A8A9-00A0C90C2004}']
    function Read(cProperties: ULONG; pPropBag: PPROPBAG2;
...
function LoadWinCodecs: Boolean;
procedure UnloadWinCodecs;

implementation

var
  WinCodecsLib: THandle;

function LoadWinCodecs: Boolean;
const
  windowscodecs = 'windowscodecs.dll';
begin
  Result := WinCodecsLib <> 0;
  if Result then Exit;
  WinCodecsLib := LoadLibrary(windowscodecs);
  Result := WinCodecsLib <> 0;
  if not Result then Exit;
  @WICMatchMetadataContent := GetProcAddress(WinCodecsLib, 'WICMatchMetadataContent');
  @WICSerializeMetadataContent := GetProcAddress(WinCodecsLib, 'WICSerializeMetadataContent');
  @WICGetMetadataContentSize := GetProcAddress(WinCodecsLib, 'WICGetMetadataContentSize');
  @WICConvertBitmapSource := GetProcAddress(WinCodecsLib, 'WICConvertBitmapSource');
  @WICCreateBitmapFromSection := GetProcAddress(WinCodecsLib, 'WICCreateBitmapFromSection');
  @WICMapGuidToShortName := GetProcAddress(WinCodecsLib, 'WICMapGuidToShortName');
  @WICMapShortNameToGuid := GetProcAddress(WinCodecsLib, 'WICMapShortNameToGuid');
  @WICMapSchemaToName := GetProcAddress(WinCodecsLib, 'WICMapSchemaToName');
end;

procedure UnloadWinCodecs;
begin
  if WinCodecsLib <> 0 then
  begin
    @WICMatchMetadataContent := nil;
    @WICSerializeMetadataContent := nil;
    @WICGetMetadataContentSize := nil;
    @WICConvertBitmapSource := nil;
    @WICCreateBitmapFromSection := nil;
    @WICMapGuidToShortName := nil;
    @WICMapShortNameToGuid := nil;
    @WICMapSchemaToName := nil;
    FreeLibrary(WinCodecsLib);
    WinCodecsLib := 0;
  end;
end;
