...
uses
  ...
  SysUtils, Forms, StdCtrls, WinTools, BaseTypes, GraphTools;
...
{ TInspectObject }
  ...
  private
    ...
    function GetEditor: TInspectorEditor;
  protected
    ...
    property Editor: TInspectorEditor read GetEditor;
    property Inspector: TCustomInspector read FInspector;
  public
...
procedure BuildColorBitmap(Bitmap: TBitmap; Color: TColor);
var
...
var
  ...
  begin
    ...
    begin
      ...
      {else
        FillRect(Rect);}
      Pen.Color := Blend(Color, 0, 50); //clBtnShadow;
      MoveTo(FSplitPos, Top);
      LineTo(FSplitPos, Bottom);
      Pen.Color := Blend(Color, clWhite, 50); //clBtnHighlight;
      MoveTo(FSplitPos + 1, Top);
      LineTo(FSplitPos + 1, Bottom);
      if Index = FNewIndex then
      begin
        if ThemePainter.Enabled then
          Pen.Color := Blend(Color, 0, 75) // clBtnShadow
        else
          Pen.Color := Blend(Color, 0, 50); // cl3DDkShadow;
        MoveTo(Left, Top);
...
begin
  ...
  ItemHeight := CalcCaptionSize(DC, 'Wg').cy + 4;
  SelectObject(DC, F);
  ReleaseDC(0, DC);
  if ItemIndex > -1 then
    UpdateControls(ItemIndex);
