...
function LoadOpenVG: Boolean;
procedure UnloadOpenVG;
...
function LoadOpenVG: Boolean;
const
  vectors = 'libAmanithVG.dll';
begin
  Result := OpenVGLib <> 0;
  if Result then Exit;
  OpenVGLib := LoadLibrary(vectors);
  Result := OpenVGLib <> 0;
  if not Result then Exit;
  @vgGetError := GetProcAddress(OpenVGLib, 'vgGetError');
  @vgFlush := GetProcAddress(OpenVGLib, 'vgFlush');
  @vgFinish := GetProcAddress(OpenVGLib, 'vgFinish');
  @vgSetf := GetProcAddress(OpenVGLib, 'vgSetf');
  @vgSeti := GetProcAddress(OpenVGLib, 'vgSeti');
  @vgSetfv := GetProcAddress(OpenVGLib, 'vgSetfv');
  @vgSetiv := GetProcAddress(OpenVGLib, 'vgSetiv');
  @vgGetf := GetProcAddress(OpenVGLib, 'vgGetf');
  @vgGeti := GetProcAddress(OpenVGLib, 'vgGeti');
  @vgGetVectorSize := GetProcAddress(OpenVGLib, 'vgGetVectorSize');
  @vgGetfv := GetProcAddress(OpenVGLib, 'vgGetfv');
  @vgGetiv := GetProcAddress(OpenVGLib, 'vgGetiv');
  @vgSetParameterf := GetProcAddress(OpenVGLib, 'vgSetParameterf');
  @vgSetParameteri := GetProcAddress(OpenVGLib, 'vgSetParameteri');
  @vgSetParameterfv := GetProcAddress(OpenVGLib, 'vgSetParameterfv');
  @vgSetParameteriv := GetProcAddress(OpenVGLib, 'vgSetParameteriv');
  @vgGetParameterf := GetProcAddress(OpenVGLib, 'vgGetParameterf');
  @vgGetParameteri := GetProcAddress(OpenVGLib, 'vgGetParameteri');
  @vgGetParameterVectorSize := GetProcAddress(OpenVGLib, 'vgGetParameterVectorSize');
  @vgGetParameterfv := GetProcAddress(OpenVGLib, 'vgGetParameterfv');
  @vgGetParameteriv := GetProcAddress(OpenVGLib, 'vgGetParameteriv');
  @vgLoadIdentity := GetProcAddress(OpenVGLib, 'vgLoadIdentity');
  @vgLoadMatrix := GetProcAddress(OpenVGLib, 'vgLoadMatrix');
  @vgGetMatrix := GetProcAddress(OpenVGLib, 'vgGetMatrix');
  @vgMultMatrix := GetProcAddress(OpenVGLib, 'vgMultMatrix');
  @vgTranslate := GetProcAddress(OpenVGLib, 'vgTranslate');
  @vgScale := GetProcAddress(OpenVGLib, 'vgScale');
  @vgShear := GetProcAddress(OpenVGLib, 'vgShear');
  @vgRotate := GetProcAddress(OpenVGLib, 'vgRotate');
  @vgMask := GetProcAddress(OpenVGLib, 'vgMask');
  @vgRenderToMask := GetProcAddress(OpenVGLib, 'vgRenderToMask');
  @vgCreateMaskLayer := GetProcAddress(OpenVGLib, 'vgCreateMaskLayer');
  @vgDestroyMaskLayer := GetProcAddress(OpenVGLib, 'vgDestroyMaskLayer');
  @vgFillMaskLayer := GetProcAddress(OpenVGLib, 'vgFillMaskLayer');
  @vgCopyMask := GetProcAddress(OpenVGLib, 'vgCopyMask');
  @vgClear := GetProcAddress(OpenVGLib, 'vgClear');
  @vgCreatePath := GetProcAddress(OpenVGLib, 'vgCreatePath');
  @vgClearPath := GetProcAddress(OpenVGLib, 'vgClearPath');
  @vgDestroyPath := GetProcAddress(OpenVGLib, 'vgDestroyPath');
  @vgRemovePathCapabilities := GetProcAddress(OpenVGLib, 'vgRemovePathCapabilities');
  @vgGetPathCapabilities := GetProcAddress(OpenVGLib, 'vgGetPathCapabilities');
  @vgAppendPath := GetProcAddress(OpenVGLib, 'vgAppendPath');
  @vgAppendPathData := GetProcAddress(OpenVGLib, 'vgAppendPathData');
  @vgModifyPathCoords := GetProcAddress(OpenVGLib, 'vgModifyPathCoords');
  @vgTransformPath := GetProcAddress(OpenVGLib, 'vgTransformPath');
  @vgInterpolatePath := GetProcAddress(OpenVGLib, 'vginterpolatePath');
  @VGPathLength := GetProcAddress(OpenVGLib, 'vgPathLength');
  @vgPointAlongPath := GetProcAddress(OpenVGLib, 'vgPointAlongPath');
  @VGPathBounds := GetProcAddress(OpenVGLib, 'vgPathBounds');
  @VGPathTransformedBounds := GetProcAddress(OpenVGLib, 'vgPathTransformedBounds');
  @vgDrawPath := GetProcAddress(OpenVGLib, 'vgDrawPath');
  @vgCreatePaint := GetProcAddress(OpenVGLib, 'vgCreatePaint');
  @vgDestroyPaint := GetProcAddress(OpenVGLib, 'vgDestroyPaint');
  @vgSetPaint := GetProcAddress(OpenVGLib, 'vgSetPaint');
  @vgGetPaint := GetProcAddress(OpenVGLib, 'vgGetPaint');
  @vgSetColor := GetProcAddress(OpenVGLib, 'vgSetColor');
  @vgGetColor := GetProcAddress(OpenVGLib, 'vgGetColor');
  @vgPaintPattern := GetProcAddress(OpenVGLib, 'vgPaintPattern');
  @vgCreateImage := GetProcAddress(OpenVGLib, 'vgCreateImage');
  @vgDestroyImage := GetProcAddress(OpenVGLib, 'vgDestroyImage');
  @vgClearImage := GetProcAddress(OpenVGLib, 'vgClearImage');
  @vgImageSubData := GetProcAddress(OpenVGLib, 'vgImageSubData');
  @vgGetImageSubData := GetProcAddress(OpenVGLib, 'vgGetImageSubData');
  @vgChildImage := GetProcAddress(OpenVGLib, 'vgChildImage');
  @vgGetParent := GetProcAddress(OpenVGLib, 'vgGetParent');
  @vgCopyImage := GetProcAddress(OpenVGLib, 'vgCopyImage');
  @vgDrawImage := GetProcAddress(OpenVGLib, 'vgDrawImage');
  @vgSetPixels := GetProcAddress(OpenVGLib, 'vgSetPixels');
  @vgWritePixels := GetProcAddress(OpenVGLib, 'vgWritePixels');
  @vgGetPixels := GetProcAddress(OpenVGLib, 'vgGetPixels');
  @vgReadPixels := GetProcAddress(OpenVGLib, 'vgReadPixels');
  @vgCopyPixels := GetProcAddress(OpenVGLib, 'vgCopyPixels');
  @vgCreateFont := GetProcAddress(OpenVGLib, 'vgCreateFont');
  @vgDestroyFont := GetProcAddress(OpenVGLib, 'vgDestroyFont');
  @vgSetGlyphToPath := GetProcAddress(OpenVGLib, 'vgSetGlyphToPath');
  @vgSetGlyphToImage := GetProcAddress(OpenVGLib, 'vgSetGlyphToImage');
  @vgClearGlyph := GetProcAddress(OpenVGLib, 'vgClearGlyph');
  @vgDrawGlyph := GetProcAddress(OpenVGLib, 'vgDrawGlyph');
  @vgDrawGlyphs := GetProcAddress(OpenVGLib, 'vgDrawGlyphs');
  @vgColorMatrix := GetProcAddress(OpenVGLib, 'vgColorMatrix');
  @vgConvolve := GetProcAddress(OpenVGLib, 'vgConvolve');
  @vgSeparableConvolve := GetProcAddress(OpenVGLib, 'vgSeparableConvolve');
  @vgGaussianBlur := GetProcAddress(OpenVGLib, 'vgGaussianBlur');
  @vgLookup := GetProcAddress(OpenVGLib, 'vgLookup');
  @vgLookupSingle := GetProcAddress(OpenVGLib, 'vgLookupSingle');
  @vgHardwareQuery := GetProcAddress(OpenVGLib, 'vgHardwareQuery');
  @vgGetString := GetProcAddress(OpenVGLib, 'vgGetString');
  @vguLine := GetProcAddress(OpenVGLib, 'vguLine');
  @vguPolygon := GetProcAddress(OpenVGLib, 'vguPolygon');
  @vguRect := GetProcAddress(OpenVGLib, 'vguRect');
  @vguRoundRect := GetProcAddress(OpenVGLib, 'vguRoundRect');
  @vguEllipse := GetProcAddress(OpenVGLib, 'vguEllipse');
  @vguArc := GetProcAddress(OpenVGLib, 'vguArc');
  @vguComputeWarpQuadToSquare := GetProcAddress(OpenVGLib, 'vguComputeWarpQuadToSquare');
  @vguComputeWarpSquareToQuad := GetProcAddress(OpenVGLib, 'vguComputeWarpSquareToQuad');
  @vguComputeWarpQuadToQuad := GetProcAddress(OpenVGLib, 'vguComputeWarpQuadToQuad');
  @vgInitContextAM := GetProcAddress(OpenVGLib, 'vgInitContextAM');
  @vgDestroyContextAM := GetProcAddress(OpenVGLib, 'vgDestroyContextAM');
  @vgResizeSurfaceAM := GetProcAddress(OpenVGLib, 'vgResizeSurfaceAM');
  @vgGetSurfaceWidthAM := GetProcAddress(OpenVGLib, 'vgGetSurfaceWidthAM');
  @vgGetSurfaceHeightAM := GetProcAddress(OpenVGLib, 'vgGetSurfaceHeightAM');
  @vgGetSurfaceFormatAM := GetProcAddress(OpenVGLib, 'vgGetSurfaceFormatAM');
  @vgGetSurfacePixelsAM := GetProcAddress(OpenVGLib, 'vgGetSurfacePixelsAM');
end;

procedure UnloadOpenVG;
begin
  if OpenVGLib = 0 then Exit;
  FreeLibrary(OpenVGLib);
  OpenVGLib := 0;
  @vgGetError := nil;
  @vgFlush := nil;
  @vgFinish := nil;
  ...
  @vgSetParameterf := nil;
  @vgSetParameteri := nil;
  @vgSetParameterfv := nil;
  @vgSetParameteriv := nil;
  @vgGetParameterf := nil;
  @vgGetParameteri := nil;
  @vgGetParameterVectorSize := nil;
  @vgGetParameterfv := nil;
  @vgGetParameteriv := nil;
  @vgLoadIdentity := nil;
  @vgLoadMatrix := nil;
  @vgGetMatrix := nil;
  @vgMultMatrix := nil;
  ...
  @vgMask := nil;
  @vgRenderToMask := nil;
  @vgCreateMaskLayer := nil;
  @vgDestroyMaskLayer := nil;
  @vgFillMaskLayer := nil;
  @vgCopyMask := nil;
  @vgClear := nil;
  @vgCreatePath := nil;
  @vgClearPath := nil;
  @vgDestroyPath := nil;
  @vgRemovePathCapabilities := nil;
  @vgGetPathCapabilities := nil;
  @vgAppendPath := nil;
  @vgAppendPathData := nil;
  @vgModifyPathCoords := nil;
  @vgTransformPath := nil;
  @vgInterpolatePath := nil;
  @VGPathLength := nil;
  @vgPointAlongPath := nil;
  @VGPathBounds := nil;
  @VGPathTransformedBounds := nil;
  @vgDrawPath := nil;
  @vgCreatePaint := nil;
  @vgDestroyPaint := nil;
  @vgSetPaint := nil;
  ...
  @vgCreateFont := nil;
  @vgDestroyFont := nil;
  @vgSetGlyphToPath := nil;
  @vgSetGlyphToImage := nil;
  @vgClearGlyph := nil;
  @vgDrawGlyph := nil;
  @vgDrawGlyphs := nil;
  @vgColorMatrix := nil;
  @vgConvolve := nil;
  @vgSeparableConvolve := nil;
  @vgGaussianBlur := nil;
  @vgLookup := nil;
  @vgLookupSingle := nil;
  @vgHardwareQuery := nil;
  ...
  @vguEllipse := nil;
  @vguArc := nil;
  @vguComputeWarpQuadToSquare := nil;
  @vguComputeWarpSquareToQuad := nil;
  @vguComputeWarpQuadToQuad := nil;
  @vgInitContextAM := nil;
  @vgDestroyContextAM := nil;
  @vgResizeSurfaceAM := nil;
  @vgGetSurfaceWidthAM := nil;
  @vgGetSurfaceHeightAM := nil;
  @vgGetSurfaceFormatAM := nil;
  @vgGetSurfacePixelsAM := nil;
end;
