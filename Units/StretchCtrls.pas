...
uses
  ...
  BaseTypes, GraphTools, MathTools, SysTools, WinTools, BlendTools;
...
type
  ...
  TStretchMode = (smNone, smLeft, smTop, smRight, smBottom, smExplode, smZoom, smBlend);
...
procedure TStretchWindow.WMSize(var Message: TWMSize);
begin
  inherited;
  if Visible then
    FShadow.Update;
...
procedure TStretchWindow.WMTimer(var Message: TWMTimer);
var
  Delta: Int64;
begin
  inherited;
  ...
  begin
    ...
    if Delta > FStretcher.FInterval then
    begin
      FShadow.Opacity := High(Byte) div 3;
      FShadow.Update;
      AlphaBlend := False;
      KillTimer(Handle, 10);
    end
    ...
    begin
      AlphaBlendValue := Round((FTimer.ElapsedTime / FStretcher.FInterval) * $FF);
      FShadow.Opacity := AlphaBlendValue div 3;
      FShadow.Update;
    end;
  end;
end;
...
begin
  ...
  DrawRect := GetUpRect;
  DrawFlatScroll(DC, drUp, DrawRect, PushStyles[FUpButtonPressed]);
  SelectClipRect(DC, DrawRect, RGN_DIFF);
  DrawRect := GetDownRect;
  DrawFlatScroll(DC, drDown, DrawRect, PushStyles[FDownButtonPressed]);
  SelectClipRect(DC, DrawRect, RGN_DIFF);
  DrawRect := ListRect;
...
begin
  inherited;
  if HandleAllocated and FShowCaption then
  begin
    Rect := GetCaptionRect;
    InvalidateRect(Handle, @Rect, True);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      I := Bottom - 5;
      Top := Bottom - 255;
      DrawGradient(Handle, DrawRect, FTitleBase, FTitleLead, drUp);
...
begin
  ...
  begin
    ...
    if odSelected in State then
      DrawMenuHighlightRect(Handle, Rect);
    InflateRect(Rect, -4, 0);
...

{
    ...
    begin
      ...
      begin
        Inc(BoxRect.Left, CalcCaptionSize(DC, FColumns[I]).cx);
