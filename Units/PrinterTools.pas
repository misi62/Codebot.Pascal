...
uses
  Classes, SysUtils, Windows, Graphics, WinSpool, Printers,
  GraphTools, BaseTypes;
...
begin
  FX := X;
  FY := Y;
  with DeviceToPoint(Vec2(X, Y)) do
    Canvas.MoveTo(X, Y);
end;
...
begin
  if S = '' then Exit;
  DC := Canvas.Handle;
  Rect := DeviceToRect(VertexRect(FX, FY - GetTextAscent, FX + Width, FY));
...
begin
  ...
  if S <> '' then
  begin
    Rect := DeviceToRect(VertexRect(FX, FY, FX + Width, FY + Height));
...
function TAdvancedPrinter.GetDpiX: Integer;
begin
  CheckPrinting(True);
  Result := GetDeviceCaps(Canvas.Handle, LOGPIXELSX);
end;

function TAdvancedPrinter.GetDpiY: Integer;
begin
  CheckPrinting(True);
