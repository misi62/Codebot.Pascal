...
//procedure AddUnit(Form: TComponent; const UnitName: string);

{ GetFormModule procdure }

//function GetFormModule(Form: TComponent): IOTAModule;
...
begin
  SetLength(S, Count);
  FillChar(PAnsiChar(S)^, Count, #0);
  Reader := Editor.CreateReader;
  Reader.GetText(Position, PAnsiChar(S), Count);
  {$IFDEF D12_UP}
  Result := UTF8ToString(PAnsiChar(S));
  {$ELSE}
  Result := S;
  {$ENDIF}
end;
