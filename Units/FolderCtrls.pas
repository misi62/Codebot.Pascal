...
uses
  ...
  CommCtrl, MathTools, BaseTypes, GraphTools, ProviderTools, FormTools, Dialogs;
...
{ TCustomFolderView }
  ...
  TCustomFolderView = class(TFramedWindow)
  private
    ...
    procedure CMFolderImages(var Msg: TMessage); message CM_FOLDERIMAGES;
...
begin
  ...
  FTextHeight := CalcCaptionSize(DC, ' ').cY;
  SelectObject(DC, F);
  ReleaseDC(0, DC);
...
procedure TCustomFolderView.CMDesignHitTest(var Msg: TCMDesignHitTest);
const
...
procedure TCustomFolderView.CMFontChanged(var Msg: TMessage);
begin
  inherited;
  Canvas.Font := Font;
  FTextHeight := Canvas.TextHeight(' ');
...
procedure TCustomFolderView.CMMouseEnter(var Msg: TMessage);
begin
  inherited;
...
procedure TCustomFolderView.CMMouseLeave(var Msg: TMessage);
begin
  inherited;
  if MouseCapture then
...
begin
  Details := PFolderItemDetails(Msg.WParam);
...
procedure TCustomFolderView.CNItemClick(var Msg: TMessage);
begin
  DoItemClick(TFolderItem(Msg.WParam));
  Msg.Result := 1;
end;

procedure TCustomFolderView.WMEraseBkgnd(var Msg: TWMEraseBkgnd);
begin
  Msg.Result := 1;
end;

procedure TCustomFolderView.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
  Msg.Result := DLGC_WANTARROWS;
end;

procedure TCustomFolderView.WMMouseWheel(var Msg: TWMMouseWheel);
var
  Delta: Integer;
begin
  if Selected <> nil then
  begin
    ...
    if (not IsRectEmpty(ButtonRect[fbScrollUp])) and (Delta = -1) then
      ...
      Selected.TopIndex := Selected.TopIndex + 1;
    if not MouseCapture then
      with Msg, ScreenToClient(Point(XPos, YPos)) do
          CaptureItem := ItemFromPoint(X, Y);
  end;
  inherited;
end;

procedure TCustomFolderView.WMTimer(var Msg: TWMTimer);
begin
  if FButtons = [] then
  begin
    KillTimer(Handle, 1);
